  ...
  Future<List<PostModel>> fetchHomeFeed() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint('Raw API response: ${response.body.substring(0, min(100, response.body.length))}...');
        ...
        if (jsonContent.trim().startsWith('Posts retrieved successfully')) {
          ...
          if (jsonStart >= 0) {
            ...
            debugPrint('Extracted JSON content starting with: ${jsonContent.substring(0, min(50, jsonContent.length))}...');
          } else {
        ...
        if (!data.containsKey('posts')) {
          debugPrint('API response missing "posts" field: ${data.keys.join(', ')}');
          return [];
        }
