...
class SearchRepository {
  Future<Map<String, dynamic>> searchPosts(
    ...
    try {
      ...
      final String endpoint =
          '/search/posts?keyword=$keyword&page=$page&limit=$limit';
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint(
          '✅ Found ${postsJson.length} posts. Total: ${pagination['total'] ?? 0},',
        );
        ...
        debugPrint(
          '❌ Failed to search posts: ${response.statusCode} - ${response.body}',
        );
        return {
          'posts': [],
          'pagination': {'total': 0, 'page': 1, 'pages': 0},
  ...
  Future<Map<String, dynamic>> searchCompanies(String keyword) async {
    final response = await RequestService.get('/companies');
    print('companies response: ${response.statusCode}');
    if (response.statusCode == 200) {
      final List data = jsonDecode(response.body);
      print('companies dataaaaa: $data');
      print('companies data length: ${data.length}');
      print('Raw company: ${jsonEncode(data.first)}');
      final filtered =
          data
              .where(
              ...
              .map((json) => Company.fromJson(json['company']))
              .toList();

      print('${filtered.length} companies found for keyword: $keyword');

      return {
        'companies': filtered,
        'pagination': {'page': 1, 'pages': 1, 'limit': filtered.length},
      };
    } else {
