...
class PostCard extends ConsumerWidget {
              ...
              if (post.isRepost == true) ...[
                Row(
                  ...
                  children: [
                    ...
                    if (post.reposterProfilePicture != null &&
                        post.reposterProfilePicture!.isNotEmpty)
                      CircleAvatar(
                        backgroundImage: NetworkImage(
                          post.reposterProfilePicture!,
                        ),
                        radius: 1.7.h,
                        onBackgroundImageError: (exception, stackTrace) {
                          debugPrint(
                            'Error loading reposter profile image: $exception',
                          );
                        },
                        child:
                            post.reposterProfilePicture!.isEmpty
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          if (post.repostDescription != null &&
                              post.repostDescription!.isNotEmpty) ...[
                          ...
                          userState.when(
                            data: (user) {
                              ...
                              final bool isMyRepost =
                                  post.isRepost == true &&
                                  post.reposterId == user.id;

                              // Determine if this is the user's own post that has been reposted
                              final bool isMyPostReposted =
                                  post.isMine == true && post.isRepost == true;

                              // Debug
                              if (post.isRepost == true) {
                                debugPrint(
                                  'Post ${post.id} - isRepost: ${post.isRepost}, isMine: ${post.isMine}, reposterId: ${post.reposterId}, userId: ${user.id}, isMyRepost: $isMyRepost, isMyPostReposted: $isMyPostReposted',
                                );
                              }

                              return Icon(
                                // Use either condition to show the reposted icon
                                (isMyRepost || isMyPostReposted)
                                    ? Icons.repeat_one
                                    : Icons.repeat,
                                size: 2.5.h,
                                color:
  ...
  Widget _buildMediaContent({
    ...
    switch (mediaType) {
      ...
      case MediaType.unknown:
        print('URL: $url');
        print(
          'Explicit media type: ${post.mediaType}',
        ); // Note: This might cause an error if post is not accessible
        final extension = _getFileExtension(url);
        print('Detected extension: $extension');
        MediaType detectedType = _getMediaType(
          url,
          explicitType: post.mediaType,
        ); // Same issue with post.mediaType
        print('Detected media type: $detectedType');
  ...
  void _showDocumentViewer(BuildContext context, String documentUrl) {
    final String googleDocsUrl =
        documentUrl.startsWith('https://docs.google.com/viewer')
            ? documentUrl
                          ...
                          ..setNavigationDelegate(
                            NavigationDelegate(
                              ...
                              onWebResourceError: (error) {
                                print(
                                  'Error loading document: ${error.description}',
                                );
                              },
                            ),
                          ),
  ...
  Widget _buildGenericUrlPreview(
    BuildContext context,
    String url,
    ThemeData theme,
  ) {
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  Row(
                    children: [
                      Icon(Icons.link, size: 2.5.h, color: AppColors.primary),
                      SizedBox(width: 1.w),
                      Expanded(
                        child: Text(
                          domain,
  ...
  void _launchUrl(BuildContext context, String url) {
    ...
    if (uri != null) {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Opening URL: $url')));
